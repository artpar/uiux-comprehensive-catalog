...
export const designingLessons: LearningLesson[] = [
  {
    ...
    options: [
      { label: "Submit → success message → close the request; staff contact the tenant only if there is a problem.", feedback: "This is compact but calls the whole task complete before triage and hides the next step. It may work only if submission itself is the user's entire goal, which needs research." },
      { label: "Submit → received status → triage outcome → appointment or needed-information path → completion and reopening route.", feedback: "This best represents the service's real states and gives the tenant a way to follow and recover. Verify that staff can keep these statuses current; a false precision would harm trust." },
      { label: "Ask tenants to phone the service after submitting so staff can confirm every stage.", feedback: "Human contact can help complex or urgent cases, but making it mandatory duplicates work and may exclude people who cannot call. It is better as a fallback unless the service cannot provide reliable status." }
    ],
    strongestOption: 1,
  ...
  },
  {
    ...
    options: [
      { label: "Show available slots grouped by day with an earliest-evening shortcut and a date control for specific-date users.", feedback: "This is a strong provisional fit because it supports the dominant task and preserves the secondary task. Test whether the extra shortcut adds clutter or hides important constraints." },
      { label: "Use a month calendar as the only entry point because booking systems usually do.", feedback: "A calendar is familiar and helps date-specific planning, but it may make the earliest viable evening hard to find. Test the real task rather than relying on convention." },
      { label: "Show only the next available evening and require users to reject it to see another slot.", feedback: "This can speed a simple one-choice task but may force repeated steps for people comparing travel, caregiving, or date constraints. It needs evidence that the narrowing helps more than it hides." }
    ],
    strongestOption: 0,
  ...
  },
  {
    ...
    options: [
      { label: "Show amount, requester, and reason with a confirmation dialog; let the notification service handle version changes.", feedback: "This can work if version freshness and eligibility are guaranteed by that service. Verify the guarantee; a dialog does not show policy exceptions or the downstream state." },
      { label: "Check current version and approver role, summarize exceptions and next approval, and block action on stale requests.", feedback: "This is strongest under the stated uncertainty. Test whether the summary supports a real decision and whether the version check remains correct across devices." },
      { label: "Require a call with the requester for every request above the threshold and record the decision afterward.", feedback: "Discussion may be useful for rare complex requests, but it imposes cost on all such requests and may not preserve the reviewed facts. A policy could require it in some contexts." }
    ],
    strongestOption: 1,
  ...
  },
  {
    ...
    options: [
      { label: "A polished static mockup of the companion app's voice settings.", feedback: "This can test labels and settings layout, but cannot represent the command, misrecognition, or hands-free recovery that drive the decision." },
      { label: "A fully engineered speech model and production hardware before talking to users.", feedback: "Production realism may eventually be necessary, but it is a costly first step for learning whether people understand and attempt the recovery path." },
